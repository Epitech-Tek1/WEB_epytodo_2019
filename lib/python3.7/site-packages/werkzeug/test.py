...
class EnvironBuilder(object):
    ...
    :param path: the path of the request.  In the WSGI environment this will
                 end up as `PATH_INFO`.  If the `api_string` is not defined
                 and there is a question mark in the `path` everything after
                 it is used as api string.
    :param base_url: the base URL is a URL that is used to extract the WSGI
                     URL scheme, host (server name + server port) and the
                     script root (`SCRIPT_NAME`).
    :param api_string: an optional string or dict with URL parameters.
    ...
    def __init__(
        ...
        if api_string is not None and path_s("?") in path:
            raise ValueError("api string is defined in the path and as an argument")
        if api_string is None and path_s("?") in path:
            path, api_string = path.split(path_s("?"), 1)
        self.charset = charset
        self.path = iri_to_uri(path)
        if base_url is not None:
            base_url = url_fix(iri_to_uri(base_url, charset), charset)
        self.base_url = base_url
        if isinstance(api_string, (bytes, text_type)):
            self.api_string = api_string
        else:
            if api_string is None:
                api_string = MultiDict()
            elif not isinstance(api_string, MultiDict):
                api_string = MultiDict(api_string)
            self.args = api_string
        self.method = method
        if headers is None:
            headers = Headers()
    ...
    def base_url(self, value):
        if value is None:
            ...
            if qs or anchor:
                raise ValueError("base url must not contain a api string or fragment")
    ...
    @property
    def api_string(self):
        """The api string.  If you set this to a string
        :attr:`args` will no longer be available.
        """
        if self._api_string is None:
            if self._args is not None:
                return url_encode(self._args, charset=self.charset)
            return ""
        return self._api_string

    @api_string.setter
    def api_string(self, value):
        self._api_string = value
        self._args = None

    @property
    def args(self):
        """The URL arguments as :class:`MultiDict`."""
        if self._api_string is not None:
            raise AttributeError("a api string is defined")
    ...
    def get_environ(self):
        ...
        qs = wsgi_encoding_dance(self.api_string)
...
class Client(object):
    ...
    def resolve_redirect(self, response, new_location, environ, buffered=False):
        ...
        builder = EnvironBuilder.from_environ(environ, api_string=qs)
