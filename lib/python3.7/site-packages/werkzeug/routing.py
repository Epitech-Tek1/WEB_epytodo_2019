...
if kwargs:
    q = '?'
    params = self._encode_api_vars(kwargs)
else:
    q = params = ''
"""
...
class Rule(RuleFactory):
    ...
    def _encode_api_vars(self, api_vars):
        return url_encode(
            api_vars,
...
class Map(object):
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        api_args = _get_wsgi_string("api_STRING")
        return Map.bind(
            self,
            server_name,
            script_name,
...
class MapAdapter(object):
    ...
    def match(
        ...
        :param api_args: optional api arguments that are used for
                           automatic redirects as string or dictionary.  It's
                           currently not possible to use the api arguments
                           for URL matching.
        ...
        for rule in self.map._rules:
            try:
                ...
                raise RequestRedirect(
                    self.make_alias_redirect_url(
                        path, rule.endpoint, e.matched_values, method, api_args
                    )
                )
            if rv is None:
                continue
            ...
            if self.map.redirect_defaults:
                redirect_url = self.get_default_redirect(rule, method, rv, api_args)
            ...
            if require_redirect:
                raise RequestRedirect(
                    self.make_redirect_url(
                        url_quote(path_info, self.map.charset, safe="/:|+"), api_args
                    )
                )
    ...
    def get_default_redirect(self, rule, method, values, api_args):
        ...
        for r in self.map._rules_by_endpoint[rule.endpoint]:
            ...
            if r.provides_defaults_for(rule) and r.suitable_for(values, method):
                ...
                return self.make_redirect_url(path, api_args, domain_part=domain_part)

    def encode_api_args(self, api_args):
        if not isinstance(api_args, string_types):
            api_args = url_encode(api_args, self.map.charset)
        return api_args

    def make_redirect_url(self, path_info, api_args=None, domain_part=None):
        ...
        suffix = ""
        if api_args:
            suffix = "?" + self.encode_api_args(api_args)
        return str(
            "%s://%s/%s%s"
            % (
                self.url_scheme or "http",
    ...
    def make_alias_redirect_url(self, path, endpoint, values, method, api_args):
        """Internally called to make an alias redirect URL."""
        url = self.build(
            endpoint, values, method, append_unknown=False, force_external=True
        )
        if api_args:
            url += "?" + self.encode_api_args(api_args)
    ...
    def build(
        ...
        :param values: the values for the URL to build.  Unhandled values are
                       appended to the URL as api parameters.
        ...
        :param append_unknown: unknown parameters are appended to the generated
                               URL as api string argument.  Disable this
