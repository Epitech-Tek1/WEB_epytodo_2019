...
def get_current_url(
    ...
    'http://localhost/'
    >>> get_current_url(env, strip_apistring=True)
    'http://localhost/script/'
    ...
    :param strip_apistring: set to `True` if you don't want the apistring.
    ...
    if not root_only:
        ...
        if not strip_apistring:
            qs = get_api_string(environ)
            if qs:
                cat("?" + qs)
    return uri_to_iri("".join(tmp))
...
def get_api_string(environ):
    """Returns the `api_STRING` from the WSGI environment.  This also takes
    ...
    :param environ: the WSGI environment object to get the api string from.
    """
    qs = wsgi_get_bytes(environ.get("api_STRING", ""))
    # api_STRING really should be ascii safe but some browsers
