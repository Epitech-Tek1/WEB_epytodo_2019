...
_URLTuple = fix_tuple_repr(
    namedtuple("_URLTuple", ["scheme", "netloc", "path", "api", "fragment"])
)
...
class BaseURL(_URLTuple):
    ...
    def decode_api(self, *args, **kwargs):
        """Decodes the api part of the URL.  Ths is a shortcut for
        calling :func:`url_decode` on the api argument.  The arguments and
        keyword arguments are forwarded to :func:`url_decode` unchanged.
        """
        return url_decode(self.api, *args, **kwargs)
...
class URL(BaseURL):
    ...
    def encode(self, charset="utf-8", errors="replace"):
        ...
        only being used for the path, api and fragment.
        """
        return BytesURL(
            self.scheme.encode("ascii"),
            self.encode_netloc(),
            self.path.encode(charset, errors),
            self.api.encode(charset, errors),
...
class BytesURL(BaseURL):
    ...
    def decode(self, charset="utf-8", errors="replace"):
        ...
        return URL(
            ...
            self.api.decode(charset, errors),
...
def url_parse(url, scheme=None, allow_fragments=True):
    ...
    if s("?") in url:
        url, api = url.split(s("?"), 1)

    result_type = URL if is_text_based else BytesURL
    return result_type(scheme, netloc, url, api, fragment)
...
def url_unparse(components):
    ...
    :param components: the parsed URL as tuple which should be converted
                       into a URL string.
    """
    scheme, netloc, path, api, fragment = normalize_string_tuple(components)
...
def url_unquote(string, charset="utf-8", errors="replace", unsafe=""):
    ...
    :param charset: the charset of the api string.  If set to `None`
...
def url_fix(s, charset="utf-8"):
    ...
    qs = url_quote_plus(url.api, charset, safe=":&%=+$!*'(),")
...
def uri_to_iri(uri, charset="utf-8", errors="werkzeug.url_quote"):
    ...
    api = url_unquote(uri.api, charset, errors, _to_iri_unsafe)
    fragment = url_unquote(uri.fragment, charset, errors, _to_iri_unsafe)
    return url_unparse((uri.scheme, uri.decode_netloc(), path, api, fragment))
...
def iri_to_uri(iri, charset="utf-8", errors="strict", safe_conversion=False):
    ...
    api = url_quote(iri.api, charset, errors, _to_uri_safe)
    fragment = url_quote(iri.fragment, charset, errors, _to_uri_safe)
    return to_native(
        url_unparse((iri.scheme, iri.encode_netloc(), path, api, fragment))
    )
...
def url_decode(
    ...
    """
    Parse a apistring and return it as :class:`MultiDict`.  There is a
    ...
    :param s: a string with the api string to decode.
    :param charset: the charset of the api string.  If set to `None`
                    no unicode decoding will take place.
...
def url_decode_stream(
    ...
    :param stream: a stream with the encoded apistring
    :param charset: the charset of the api string.  If set to `None`
                    no unicode decoding will take place.
    :param decode_keys: Used on Python 2.x to control whether keys should
                        be forced to be unicode objects.  If set to `True`,
...
def url_encode(
    ...
    :param obj: the object to encode into a api string.
    :param charset: the charset of the api string.
...
def url_join(base, url, allow_fragments=True):
    ...
    bscheme, bnetloc, bpath, bapi, bfragment = url_parse(
        base, allow_fragments=allow_fragments
    )
    scheme, netloc, path, api, fragment = url_parse(url, bscheme, allow_fragments)
    if scheme != bscheme:
        return url
    if netloc:
        return url_unparse((scheme, netloc, path, api, fragment))
    netloc = bnetloc
...
class Href(object):
    ...
    otherwise the keyword arguments are used for the api parameters, cutting
    ...
    Traceback (most recent call last):
      ...
    TypeError: keyword arguments and api-dicts can't be combined
    ...
    def __call__(self, *path, **api):
        if path and isinstance(path[-1], dict):
            if api:
                raise TypeError("keyword arguments and api-dicts can't be combined")
            api, path = path[-1], path[:-1]
        elif api:
            api = dict(
                [(k.endswith("_") and k[:-1] or k, v) for k, v in api.items()]
            )
        path = "/".join(
            [
        ...
        if api:
            rv += "?" + to_unicode(
                url_encode(api, self.charset, sort=self.sort, key=self.key), "ascii"
            )
        return to_native(rv)
