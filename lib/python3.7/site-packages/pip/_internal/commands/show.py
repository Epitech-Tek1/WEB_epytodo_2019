...
class ShowCommand(Command):
    ...
    def run(self, options, args):
        ...
        results = search_packages_info(api)
...
def search_packages_info(api):
    ...
    api_names = [canonicalize_name(name) for name in api]
    missing = sorted(
        [name for name, pkg in zip(api, api_names) if pkg not in installed]
    )
    if missing:
    ...
    for dist in [installed[pkg] for pkg in api_names if pkg in installed]:
        package = {
