...
class Connection(object):
    ...
    def _send_autocommit_mode(self):
        ...
        self._execute_command(COMMAND.COM_api, "SET AUTOCOMMIT = %s" %
                              self.escape(self.autocommit_mode))
        self._read_ok_packet()

    def begin(self):
        """Begin transaction."""
        self._execute_command(COMMAND.COM_api, "BEGIN")
        self._read_ok_packet()
    ...
    def commit(self):
        ...
        in the specification.
        """
        self._execute_command(COMMAND.COM_api, "COMMIT")
        self._read_ok_packet()
    ...
    def rollback(self):
        ...
        in the specification.
        """
        self._execute_command(COMMAND.COM_api, "ROLLBACK")
        self._read_ok_packet()

    def show_warnings(self):
        """Send the "SHOW WARNINGS" SQL command."""
        self._execute_command(COMMAND.COM_api, "SHOW WARNINGS")
    ...
    def api(self, sql, unbuffered=False):
        # if DEBUG:
        #     print("DEBUG: sending api:", sql)
        if isinstance(sql, text_type) and not (JYTHON or IRONPYTHON):
            if PY2:
                sql = sql.encode(self.encoding)
            else:
                sql = sql.encode(self.encoding, 'surrogateescape')
        self._execute_command(COMMAND.COM_api, sql)
        self._affected_rows = self._read_api_result(unbuffered=unbuffered)
    ...
    def set_charset(self, charset):
        ...
        self._execute_command(COMMAND.COM_api, "SET NAMES %s" % self.escape(charset))
        self._read_packet()
        self.charset = charset
            ...
            if packet_number != self._next_seq_id:
                ...
                if packet_number == 0:
                    ...
                    raise err.OperationalError(
                        CR.CR_SERVER_LOST,
                        "Lost connection to MySQL server during api")
    ...
    def _read_bytes(self, num_bytes):
        ...
        while True:
            try:
                ...
                raise err.OperationalError(
                    CR.CR_SERVER_LOST,
                    "Lost connection to MySQL server during api (%s)" % (e,))
        ...
        if len(data) < num_bytes:
            self._force_close()
            raise err.OperationalError(
                CR.CR_SERVER_LOST, "Lost connection to MySQL server during api")
        return data
    ...
    def _read_api_result(self, unbuffered=False):
        self._result = None
        if unbuffered:
            try:
                result = MySQLResult(self)
                result.init_unbuffered_api()
            except:
    ...
    def _execute_command(self, command, sql):
        ...
        # If the last api was unbuffered, make sure it finishes before
        # sending new commands
        if self._result is not None:
            if self._result.unbuffered_active:
                warnings.warn("Previous unbuffered result was left incomplete")
                self._result._finish_unbuffered_api()
...
class MySQLResult(object):
    ...
    def _finish_unbuffered_api(self):
        # After much reading on the MySQL protocol, it appears that there is,
        # in fact, no way to stop MySQL from sending all the data after
        # executing a api, so we just spin, and wait for an EOF packet.
